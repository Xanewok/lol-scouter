...
game::game(cJSON *root, int summ_id, bool &desired)
{
	...
	int map_id = cJSON_GetObjectItem(root, "mapId")->valueint;
	...
	for (int j = 0; j < cJSON_GetArraySize(fellow_players_json); ++j) {
		cJSON *player_json = cJSON_GetArrayItem(fellow_players_json, j); // player_json to cały wpis gracza
	...
	cJSON *raw_statistics = cJSON_GetObjectItem(root, "stats"); // raw_statistics to zbiór statów
	...
	stats->gold = cJSON_GetObjectItem(raw_statistics, "goldEarned")->valueint;
